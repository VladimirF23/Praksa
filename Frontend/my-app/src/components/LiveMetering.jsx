        ...
        >
            ...
            {debugVisible && (
                <div className="absolute top-14 left-4 bg-white/90 p-4 rounded shadow-lg max-w-sm z-40">
                    ...
                    <p>
                        Status:{" "}
                        <span
                            className={`font-semibold ${status === "connected"
                                ? "text-green-500"
                                : status === "connecting"
                                    ? "text-yellow-500"
                                    : "text-red-500"
            ...
            {liveData && (
                <>
                    ...
                    <div
                        className={`absolute flex flex-col items-center justify-center border-[6px] border-green-500 bg-white text-xs font-bold text-gray-900 shadow-lg ${liveData.solar_production_kw > 0 ? "pulse" : ""
                            }`}
                        style={{
                            top: "10%",
                            left: "45%",
                    ...
                    <div
                        className={`absolute flex flex-col items-center justify-center border-[4px] border-blue-500 bg-white text-xs font-bold text-gray-900 shadow-lg ${liveData.household_consumption_kw > 0 ? "pulse" : ""
                            }`}
                        style={{
                  ...
                  ${liveData.battery_loss_kw > 0
                    ...
                    <div
                        className={`absolute flex flex-col items-center justify-center border-[4px] border-orange-500 bg-white text-xs font-bold text-gray-900 shadow-lg ${liveData.grid_contribution_kw > 0 ? "pulse" : ""
                            }`}
                        style={{
                            top: "25%",
                            left: "20%",
                    ...
                    <div className="absolute left-0 w-full bg-white/90 border-t border-gray-300 shadow-lg p-4 max-h-64 overflow-y-auto"
                        style={{
                          bottom: "-200px", // pushes the panel 200px below the visible house area
                        }}
                      >
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                            {iotDevices.map((device, index) => (
                                ...
                                >
                                    <div>
                                        {/* Smaller title */}
                                        <h4 className="text-base font-semibold">{device.device_name}</h4>
                                        <p className="text-sm text-gray-500">{device.device_type}</p>
                                        <p className="text-sm">
                                            Power: {device.base_consumption_watts} W
                                        </p>
                                        <p
                                            className={`mt-1 font-bold ${device.current_status === "on"
                                                ? "text-green-600"
                                                : "text-red-600"
                                                }`}
                                        >
                                        ...
                                        {/* Priority Level */}
                                        <p
                                            className={`text-sm mt-1 font-medium ${device.priority_level === "critical"
                                                ? "text-red-700"
                                    ...
                                    {device.is_smart_device && (
                                        ...
                                        <button
                                            ...
                                            className={`mt-2 py-1 px-3 rounded ${device.current_status === "on"
