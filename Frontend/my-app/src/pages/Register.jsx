...
// Definišemo priority levels za dropdown
const IOT_PRIORITY_LEVELS = [
...
const Register = () => {
    ...
    const [formData, setFormData] = useState({
        ...
        isBatteryCustom: false, // Da li korisnik unosi ručne vrednosti za bateriju
        customBattery: {
        ...
        customIotConsumptionWatts: {}, // { id: vrednost, ... } za ručni unos potrošnje
        
        // NOVO: Lista za ručno dodate IoT uređaje
        customIotDevices: [] // Primer: [{ id: 'custom_123', device_name: 'Moj...', ... }]
    });
    ...
    // --- NOVI HANDLERI ZA CUSTOM IoT UREĐAJE ---

    // 1. Dodaje novi, prazan custom uređaj u listu
    const handleAddCustomIot = () => {
        const newDevice = {
            id: `custom_${Date.now()}`, // Privremeni ID za React key i state management
            device_name: 'Novi Uređaj',
            device_type: 'Other',
            base_consumption_watts: '100',
            priority_level: 'low',
            is_smart_device: false
        };
        setFormData(prev => ({
            ...prev,
            customIotDevices: [...prev.customIotDevices, newDevice]
        }));
    };

    // 2. Uklanja custom uređaj iz liste
    const handleRemoveCustomIot = (id) => {
        setFormData(prev => ({
            ...prev,
            customIotDevices: prev.customIotDevices.filter(device => device.id !== id)
        }));
    };

    // 3. Menja podatke za specifični custom uređaj u listi
    const handleCustomIotChange = (id, e) => {
        const { name, value, type, checked } = e.target;
        const val = type === 'checkbox' ? checked : value; // Posebna provera za checkbox

        setFormData(prev => ({
            ...prev,
            customIotDevices: prev.customIotDevices.map(device =>
                device.id === id ? { ...device, [name]: val } : device
            )
        }));
    };
    ...
    const handleSubmit = async (e) => {
        ...
        // Logika za BATERIJU (ostaje ista)
        ...
        // Logika za IoT UREĐAJE (AŽURIRANO)

        // A. Mapiranje PREDEFINISANIH IoT uređaja (kao i pre)
        ...
        // B. NOVO: Mapiranje RUČNO DODATIH IoT uređaja
        const customDevicesPayload = formData.customIotDevices.map(d => ({
            device_name: d.device_name || 'Custom Device',
            device_type: d.device_type || 'Other',
            base_consumption_watts: parseFloat(d.base_consumption_watts) || 0,
            priority_level: d.priority_level || 'low',
            current_status: 'off',
            is_smart_device: d.is_smart_device // Ovo je boolean iz state-a
        }));
        ...
        const apiPayload = {
            ...
            // AZURIRANO: Kombinacija predefinisanih i rucno dodatih uredjaja
            iot_devices: [...selectedIotDevices, ...customDevicesPayload]
        };
    ...
    return (
        <div className="flex justify-center items-center min-h-screen bg-gray-100 p-4">
            <form onSubmit={handleSubmit} className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-2xl space-y-6">
                ...
                {/* 1. KORAK: Osnovni podaci (Bez promena) */}
                <fieldset className="border p-4 rounded-lg space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="username" id="username" value={formData.username} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        {/* Email */}
                        <div>
                            <label htmlFor="email" className="block text-sm font-medium text-gray-700">Email</label>
                            <input type="email" name="email" id="email" value={formData.email} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        {/* Password */}
                        <div>
                            <label htmlFor="password" className="block text-sm font-medium text-gray-700">Lozinka</label>
                            <input type="password" name="password" id="password" value={formData.password} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        {/* Velicina Kuce (m²) */}
                        <div>
                            <label htmlFor="house_size_sqm" className="block text-sm font-medium text-gray-700">Veličina Kuće (m²)</label>
                            <input type="number" name="house_size_sqm" id="house_size_sqm" value={formData.house_size_sqm} onChange={handleChange} required min="1" step="0.1" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="num_household_members" id="num_household_members" value={formData.num_household_members} onChange={handleChange} required min="1" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        {/* Latituda */}
                        <div>
                            <label htmlFor="latitude" className="block text-sm font-medium text-gray-700">Geografska Širina (Latituda)</label>
                            <input type="number" name="latitude" id="latitude" value={formData.latitude} onChange={handleChange} required step="any" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        {/* Longituda */}
                        <div>
                            <label htmlFor="longitude" className="block text-sm font-medium text-gray-700">Geografska Dužina (Longituda)</label>
                            <input type="number" name="longitude" id="longitude" value={formData.longitude} onChange={handleChange} required step="any" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                    </div>
                </fieldset>

                {/* 2. KORAK: Solar System Konfiguracija (Bez promena) */}
                <fieldset className="border p-4 rounded-lg space-y-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <input type="text" name="system_name" id="system_name" value={formData.system_name} onChange={handleChange} required className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="total_panel_wattage_wp" id="total_panel_wattage_wp" value={formData.total_panel_wattage_wp} onChange={handleChange} required min="100" step="any" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="inverter_capacity_kw" id="inverter_capacity_kw" value={formData.inverter_capacity_kw} onChange={handleChange} required min="0.1" step="any" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        {/* Nagib (Tilt Degrees) */}
                        <div>
                            <label htmlFor="tilt_degrees" className="block text-sm font-medium text-gray-700">Nagib Panela (Stepeni)</label>
                            <input type="number" name="tilt_degrees" id="tilt_degrees" value={formData.tilt_degrees} onChange={handleChange} required min="0" max="90" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="azimuth_degrees" id="azimuth_degrees" value={formData.azimuth_degrees} onChange={handleChange} required min="0" max="360" className="mt-1 block w-full border border-gray-300 rounded-md p-2" />
                        </div>
                    </div>
                    ...
                    {/* USLOVNO PRIKAZIVANJE BATERIJE (Bez promena) */}
                    {formData.system_type === 'grid_tied_hybrid' && (
                        <div className="border-t pt-4">
                            <h3 className="text-md font-semibold mb-2">Izbor Baterije</h3>
                            <div className="flex items-center space-x-4 mb-4">
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input type="radio" name="battery_choice" checked={!formData.isBatteryCustom} onChange={() => setFormData(prev => ({ ...prev, isBatteryCustom: false }))} className="h-4 w-4 text-green-600 border-gray-300" />
                                    <span className="text-sm font-medium text-gray-700">Koristi predefinisane modele</span>
                                </label>
                                <label className="flex items-center space-x-2 cursor-pointer">
                                    <input type="radio" name="battery_choice" checked={formData.isBatteryCustom} onChange={() => setFormData(prev => ({ ...prev, isBatteryCustom: true }))} className="h-4 w-4 text-blue-600 border-gray-300" />
                ...
                <fieldset className="border p-4 rounded-lg space-y-4">
                    ...
                    <div className="grid grid-cols-1 gap-4">
                        {PREDEFINED_IOT_DEVICES.map(device => (
                            <div key={device.id} className="flex flex-col border rounded-lg p-3 bg-white">
                                ...
                                {/* Ručni unos potrošnje (Prikazuje se samo ako je uredjaj izabran) */}
                    ...
                    {/* B. NOVO: RUCNO DODATI UREDJAJI */}
                    <div className="border-t pt-4 space-y-4">
                        <h4 className="text-md font-semibold text-gray-700">Dodaj Ručno IoT Uređaj</h4>

                        {/* Lista rucno dodatih uređaja */}
                        {formData.customIotDevices.map((device, index) => (
                            <div key={device.id} className="p-4 border rounded-lg bg-gray-50 space-y-3 relative">
                                <div className="flex items-center space-x-3">
                                    <img src="/custom_iot.png" alt="Custom IoT" className="h-8 w-8 object-contain" />
                                    <h5 className="text-md font-medium">Custom Uređaj #{index + 1}</h5>
                                </div>
                                
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                    {/* Naziv Uređaja */}
                                    <div>
                                        <label className="block text-xs font-medium text-gray-700">Naziv Uređaja</label>
                                        <input
                                            type="text"
                                            name="device_name"
                                            value={device.device_name}
                                            onChange={(e) => handleCustomIotChange(device.id, e)}
                                            required
                                            className="mt-1 block w-full text-sm border-gray-300 rounded-md p-2"
                                        />
                                    </div>
                                    {/* Tip Uređaja */}
                                    <div>
                                        <label className="block text-xs font-medium text-gray-700">Tip Uređaja (npr. 'Light', 'Appliance')</label>
                                        <input
                                            type="text"
                                            name="device_type"
                                    ...
                                    {/* Potrošnja (W) */}
                                    <div>
                                        <label className="block text-xs font-medium text-gray-700">Potrošnja (W)</label>
                                        <input
                                            type="number"
                                            name="base_consumption_watts"
                                            value={device.base_consumption_watts}
                                            onChange={(e) => handleCustomIotChange(device.id, e)}
                                            required
                                            min="0"
                                            step="1"
                                    ...
                                    {/* Prioritet */}
                                    <div>
                                        <label className="block text-xs font-medium text-gray-700">Prioritet</label>
                                        <select
                                            name="priority_level"
                                ...
                                {/* Smart Device Checkbox */}
                                <div className="flex items-center space-x-2">
                                     <input
                                        type="checkbox"
                                        name="is_smart_device"
                                        id={`smart-${device.id}`}
                                        checked={device.is_smart_device}
                                        onChange={(e) => handleCustomIotChange(device.id, e)}
                                        className="h-4 w-4 text-indigo-600 border-gray-300 rounded"
                                    />
                                    <label htmlFor={`smart-${device.id}`} className="text-sm font-medium text-gray-700">
                                        Da li je "Smart" Uređaj?
                                    </label>
                                </div>
                                ...
                                <button
                                    type="button"
                                    onClick={() => handleRemoveCustomIot(device.id)}
                        ...
                        {/* Dugme za dodavanje novog custom uredjaja */}
                        <button
                            type="button"
                            onClick={handleAddCustomIot}
                            className="w-full py-2 bg-blue-500 text-white font-semibold rounded-lg hover:bg-blue-600 transition"
                        >
                            + Dodaj Još Jedan Custom Uređaj
                        </button>
                    </div>
                </fieldset>
